<div class="w-full max-w-[85rem] py-10 px-4 sm:px-6 lg:px-8 mx-auto">
        ...
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
            <div class="p-4 md:p-5 flex gap-x-4">
                ...
                <div class="grow">
                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        <span
                            class="{{ $order->status == 'new' ? 'bg-blue-500' : ($order->status == 'shipped' ? 'bg-yellow-500' : ($order->status == 'delivered' ? 'bg-green-500' : 'bg-gray-500')) }} py-1 px-3 rounded text-white shadow">
                            {{$order->status}}
                        </span>
                    </div>
                </div>
        ...
        <div class="flex flex-col bg-white border shadow-sm rounded-xl dark:bg-slate-900 dark:border-gray-800">
            <div class="p-4 md:p-5 flex gap-x-4">
                ...
                <div class="grow">
                    ...
                    <div class="mt-1 flex items-center gap-x-2">
                        <span
                            class="{{ $order->payment_status == 'paid' ? 'bg-green-500' : ($order->payment_status == 'pending' ? 'bg-yellow-500' : ($order->payment_status == 'failed' ? 'bg-red-500' : 'bg-gray-500')) }} py-1 px-3 rounded text-white shadow">
        ...
        <div class="md:w-3/4">
                    ...
                    <tbody>
                        @foreach ($order_items as $item)
                            <tr wire:key="{{$item->id}}">
                                ...
                                <td class="py-4">{{Number::currency($item->product->price, 'IDR')}}</td>
                                <td class="py-4">
            ...
            <div class="bg-white overflow-x-auto rounded-lg shadow-md p-6 mb-4">
                ...
                <div class="flex justify-between items-center">
                    <div>
                        <p>{{$address->street_address}}, {{$address->city}}, {{$address->state}}, {{$address->zip_code}}
                        </p>
                    </div>
                    <div>
    ...
    <a class="w-full sm:w-auto whitespace-nowrap p-3 inline-flex justify-center items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600"
        wire:click="downloadPDF">Download PDF</a>
</div>